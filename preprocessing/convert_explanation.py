...
parser.add_argument("--data", type=str, default='./', help="path to preprocessed data")
parser.add_argument("--question", type=str, default='./data/', help="path to GQA questions and scene graphs")
parser.add_argument("--bbox", type=str, default='./data/extracted_features/box', help="path to bounding box information for UpDown features")
...
for split in ['train', 'val']:
    ...
    question = json.load(open(os.path.join(args.question, 'questions', split + '_balanced_questions.json')))
    ...
    for idx, qid in enumerate(explanation):
        ...
        if (idx + 1) % 10000 == 0:
            print('Processed %d out of %d samples in the %s set, time spent: %.2f' % (
            idx + 1, len(explanation), split, time.time() - start))
