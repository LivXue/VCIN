...
def create_inputs(splits, output):
    ...
    for split in splits:
        ...
        #        data = json.load(f)
        # else:
        with open('{}.json'.format(split)) as f:
            data = json.load(f)
            print("loading {}".format('{}.json'.format(split)))

        count = 0
        for idx, entry in tqdm(enumerate(data)):
            # for prog in entry[2]:
            #programs = entry[0] # for test
            programs = ''.join(entry[1])  # train
            ...
            #results.append([entry[0], entry[1], returns, inputs, connection, entry[-2], entry[-1]])    # for test
            results.append([''.join(entry[1]), ' '.join(entry[0]), returns, inputs, connection, entry[-2], entry[-1]]) #for train
            #("finished {}/{} \r".format(idx, len(data)))

    with open(output, 'w') as f:
        json.dump(results, f)


arg = 'create_pred_inputs'  # sys.argv[1]
if arg == 'trainval_all':
    raw_data = {}
    ...
    with open('../gqa_questions/train_balanced_questions.json') as f:
        raw_data = json.load(f)
    with open('../gqa_questions/val_balanced_questions.json') as f:
        raw_data.update(json.load(f))
    preprocess(raw_data, 'questions/trainval_balanced_programs.json')
    with open('../gqa_questions/testdev_balanced_questions.json') as f:
    ...
    create_inputs(['train_balanced_pairs'], 'train_balanced_program.json')
