...
def to_singular(w_list):
    if not isinstance(w_list, list):
        return to_singular([w_list])[0]
    ...
    for i in range(len(w_list)):
        ...
        w_list[i] = ' '.join(tokens).replace(' ,', ',').replace('( ', '(').replace(' )', ')').replace('# ', '#').replace('@ ', '@')
...
for split in ['train', 'val']:
    ...
    for idx, qid in enumerate(explanation):
        ...
        processed_data[qid] = to_singular(cur_exp)

        if (idx + 1) % 50000 == 0:
            print('Finished %d out of %d questions in %s split, time spent: %.2f' % (
